...
const sampleEvaluation = {
  ...
  context: new ClientContext()
    .setInt("organization_id", 3)
    .setString("state", "texas"),
}

const resolvedSampleEvaluation = {
  namespaceName: "default",
  configName: "example",
  evaluationType: EvaluationType.BOOL,
  result: true,
...
function Evaluation({ config, title }: DisplayProps) {
...
export const LekkoConfigDisplay: React.FC<LekkoConfigDisplayProps> = ({
  title,
  config,
  result,
}) => {
    ...
    <div className="p-8 bg-white shadow-md rounded-lg flex flex-col items-start">
      ...
      <div className="mb-2">
        ...
        <div className="bg-gray-100 p-2 rounded">
          {JSON.stringify(config?.context?.data, null, 2)}
        </div>
      </div>
      <div className="mb-2">
...
const EditableConfig: React.FC<Props> = ({ config }) => {
  ...
  const [pairs, setPairs] = useState<Pair[]>([
    { key: "", type: ContextType.STRING, value: "" },
  ])
      ...
      {pairs.map((pair, index) => (
        <div key={index} className="mb-4 flex gap-2 items-center">
          ...
          <select
            ...
            value={pair.type}
            onChange={(e) => {
              handleTypeChange(index, e.target.value as ContextType)
            }}
          >
    ...
    <div className="App flex flex-col items-start">
      <Suspense fallback={<Fallback />}>
        <LekkoPersistedConfigProvider
          fallback={<Fallback />}
          backupResolvedDefaultConfigs={[resolvedSampleEvaluation]}
        >
          <div className="mb-5">
            <Evaluation
              config={sampleEvaluation}
              title="Preloaded provider evaluation"
            />
          </div>
          ...
          {show && (
            <div>
              <Suspense
                fallback={
                  <div className="mt-4">
                    Loading inner suspense component...
                  </div>
                }
              >
                <Evaluation
                  config={sampleEvaluation2}
                  title="Suspenseful inner component evaluation"
                />
              </Suspense>
            </div>
          )}
          <div className="flex flex-col items-start p-3 mt-4">
            ...
            >
              {show ? "Hide inner component" : "Show inner component"}
            </button>
          </div>
        </LekkoPersistedConfigProvider>
      </Suspense>
